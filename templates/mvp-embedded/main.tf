...
#   - /apps/copilot → Terminal web (ttyd) con el binario `copilot` (GitHub Copilot
#                     CLI, @github/copilot) ya lanzado (puerto 7682)
#
...
resource "coder_agent" "main" {
  ...
  env = merge(local.llm_env, {
    ...
    # GH_TOKEN lo usa el binario `copilot` para autenticar con la cuenta del user.
  ...
  startup_script = <<-EOT
    ...
    # --- GitHub Copilot CLI (binario `copilot`, @github/copilot) — asistente adicional ---
    # Convive con Claude Code. Se autentica con la cuenta GitHub del usuario
    # (GITHUB_TOKEN/GH_TOKEN del external-auth); requiere suscripción a Copilot.
    # Si el token no basta, dentro de `copilot` se hace /login una vez.
    if ! command -v copilot >/dev/null 2>&1; then
      sudo npm install -g @github/copilot >/tmp/copilot-install.log 2>&1 || \
        npm install -g @github/copilot >/tmp/copilot-install.log 2>&1 || true
    fi
    ...
    # Terminal adicional que arranca el asistente `copilot` (como claude); si no
    # está listo (o requiere /login), cae a bash.
    ttyd -p 7682 -W -t titleFixed='GitHub Copilot' bash -lc 'cd ~/workspace && (copilot || bash)' >/tmp/ttyd-copilot.log 2>&1 &
  EOT
